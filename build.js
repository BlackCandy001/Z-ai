/**
 * build.js — Production Build Script (v8.0.0) [MODIFIED: added panel.js/panel.html for 8.0]
 * 
...
const EXTENSION_JS_FILES = ['content.js', 'inject.js', 'popup.js', 'panel.js', 'background.js', 'send-engine.js', 'dom-optimizer.js'].filter(f => fs.existsSync(path.join(EXTENSION_DIR, f)));
const EXTENSION_ASSET_FILES = ['manifest.json', 'rules.json', 'popup.html', 'panel.html'].filter(f => fs.existsSync(path.join(EXTENSION_DIR, f)));
...
function cleanDir(dir) {
    if (fs.existsSync(dir)) {
        try {
            ...
            // Nếu bị khóa thư mục gốc (EBUSY), xóa từng file con bên trong
            try {
                ...
                for (const entry of entries) {
                    ...
                    try {
                        ...
                        // ignore individual locked file if running
                    }
                }
            } catch (err) {}
        }
...
function step3_obfuscate_extension() {
    ...
    // Copy Icon folder
    const iconDir = path.join(EXTENSION_DIR, 'Icon');
    if (fs.existsSync(iconDir)) {
        const destIcon = path.join(DIST_EXTENSION, 'Icon');
        try {
            copyDirRecursive(iconDir, destIcon);
            console.log('  ✅ Icon/ → copied');
        } catch (e) {
            console.log(`  ⚠️ Could not copy Icon: ${e.message}`);
        }
    }
...
function step5_finalize(pkgSuccess) {
    ...
    // Always make sure release folder has extension + README
    const releaseExtDir = path.join(DIST_RELEASE, 'extension');
    ensureDir(DIST_RELEASE);
    cleanDir(releaseExtDir);
    copyDirRecursive(DIST_EXTENSION, releaseExtDir);
    const releaseReadmePath = path.join(DIST_RELEASE, 'README.txt');
    fs.writeFileSync(releaseReadmePath, `============================================
  Z.AI Bridge v8.0.0 (Standalone EXE)
============================================
...
============================================
`);
    console.log('  ✅ Extension + README synced to release/');
    ...
    fs.writeFileSync(batPath, `@echo off\necho Starting Z.AI Bridge Server v8.0.0...\nnode server\\loader.js\npause\n`);
...
async function main() {
    console.log('============================================================');
    console.log('  Z.AI Bridge — Production Build (v8.0.0)');
    console.log('============================================================');
