...
// ============================================================
function zenRouter(chatEngine, mappingService, getInitStatus) {
    ...
    router.get('/v1/usage', (req, res) => {
        ...
        const perAccountRateLimits = chatEngine.rateLimiter.getAllStatuses();
        res.json({
            success: true,
            data: summary,
            per_account_rate_limits: perAccountRateLimits,
            active_account: chatEngine.getActiveAccount(),
        });
    });
    ...
    const handleMessages = async (req, res) => {
        ...
        // [NEW 8.0] Rate limiting — per-account HOLD/HARD REJECT
        // Server nhận activeUserId từ extension qua account_info WS message
        const activeUserId = chatEngine.getActiveAccount() || 'unknown';
        const rateLimitResult = await chatEngine.rateLimiter.acquire(activeUserId);
        if (!rateLimitResult.allowed) {
            const retrySec = Math.ceil((rateLimitResult.retryAfterMs || 60000) / 1000);
            const reason = rateLimitResult.reason || 'unknown';
            if (reason === 'per_hour_exceeded') {
                console.log(`[Zen Route] 🚫 HARD REJECT: Per-hour limit exceeded for account ${activeUserId}.`);
                res.status(429).set('Retry-After', String(retrySec)).json({
                    error: {
                        code: 'per_hour_limit_exceeded',
                        message: `Đã vượt ngưỡng ${chatEngine.rateLimiter.getStatus(activeUserId).maxRequestsPerHour} req/giờ. Server từ chối gửi tin. Vui lòng thử lại sau ${retrySec} giây.`,
                        retry_after_seconds: retrySec,
                    }
                });
                return;
            }
            if (reason === 'waf_cooldown') {
                console.log(`[Zen Route] 🚫 HARD REJECT: WAF cooldown active for account ${activeUserId}.`);
                res.status(429).set('Retry-After', String(retrySec)).json({
                    error: {
                        code: 'waf_cooldown',
                        message: `Z.AI đang block (WAF). Cooldown ${retrySec} giây. Vui lòng thử lại sau.`,
                        retry_after_seconds: retrySec,
                    }
                });
                return;
            }
            // per_minute_exceeded (HOLD đã xử lý bên trong acquire()) — nếu vẫn fail thì reject
            console.log(`[Zen Route] 🚫 Rate limit (${reason}) for account ${activeUserId}. Retry after ${retrySec}s.`);
            res.status(429).set('Retry-After', String(retrySec)).json({
                error: {
                    code: reason,
                    message: `Rate limit exceeded. Vui lòng thử lại sau ${retrySec} giây.`,
                    retry_after_seconds: retrySec,
                }
            });
            return;
        ...
        try {
            if (stream !== false) {
                // ---- SSE streaming mode ----
                (0, sse_1.writeSSEHeaders)(res, chatEngine.rateLimiter);
                ...
                const { addToken, flush: flushPending } = (0, sse_1.createTokenFlusher)(res);
                ...
                try {
                    ...
                    await chatEngine.chat(prompt, (token) => {
                        ...
                        let output = '';
                        let i = 0;
                        // ✅ Fix question-render: theo dõi quote để '>' bên trong attribute
                        // (vd: label="a > b") không đóng tag sớm → Zen parser không bị skip question
                        let inQuote = false;
                        let quoteChar = '';
                        while (i < buffer.length) {
                            ...
                            else if (inTag && !inQuote && (ch === '"' || ch === "'")) {
                                inQuote = true;
                                quoteChar = ch;
                                i++;
                            }
                            else if (inTag && inQuote && ch === quoteChar) {
                                inQuote = false;
                                quoteChar = '';
                                i++;
                            }
                            else if (inTag && !inQuote && ch === '>') {
                                const tag = buffer.substring(tagStart, i + 1);
                                const fixedTag = (0, sanitizer_1.sanitizeToken)(tag);
                        ...
                        if (output) {
                            const sanitized = (0, sanitizer_1.sanitizeToken)(output);
                    ...
                    if (bufferParts.length > 0) {
                        const final = (0, sanitizer_1.sanitizeToken)(bufferParts.join(''));
                        if (final) {
                    ...
                    (0, sse_1.writeUsageEvent)(res, finalUsage, resolvedConversationId);
                    console.log(`\n--- [STREAM END] ---\n`);
                }
                catch (err) {
                    console.error(`\n[Stream Error]: ${err.message || String(err)}`);
                    if ((0, sanitizer_1.isWAFError)(err.message || String(err))) {
                ...
                try {
                    ...
                    await chatEngine.chat(prompt, (token) => {
                        accumulatedContent += (0, sanitizer_1.sanitizeToken)(token);
                    }, resolvedConversationId, isNewChat, useSearch);
                    const finalContent = (0, sanitizer_1.sanitizeToken)(accumulatedContent);
                    ...
                    res.json({
                        ...
                        search_results: searchResults ? searchResults.map((r, i) => ({
                            index: i + 1,
                            title: r.title || '',
                            url: r.url || r.link || '',
                            snippet: r.snippet || r.content || '',
