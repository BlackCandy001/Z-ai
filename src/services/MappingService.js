...
function getMappingFilePath() {
    ...
    if (isPackaged) {
        ...
        if (normalizedDir.endsWith('/dist/server') || normalizedDir.endsWith('/dist/services')) {
            baseDir = path.join(__dirname, '../../');
        }
        else if (normalizedDir.endsWith('/src/services') || normalizedDir.endsWith('/src')) {
            baseDir = path.join(__dirname, '../../');
        }
        else {
            baseDir = __dirname;
    ...
    return path.join(baseDir, 'conversation-mapping.json');
}
const CONVERSATION_MAPPING_FILE = getMappingFilePath();
