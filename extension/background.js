...
function connectWS() {
    ...
    ws.onclose = () => {
        ...
        if (reconnectAttempts < MAX_PHASE1_ATTEMPTS) {
            ...
            reconnectAttempts++;
            console.log(`[Background] Attempting WS reconnect (Phase 1) ${reconnectAttempts}/${MAX_PHASE1_ATTEMPTS} in ${Math.round(delay)}ms`);
            scheduleReconnect(delay);
        } else if (reconnectAttempts < MAX_TOTAL_ATTEMPTS) {
            reconnectAttempts++;
            const delay = 10 * 60 * 1000; // 10 phút interval cho Phase 2
            console.log(`[Background] ⚠️ WS server unreachable. Attempting Phase 2 reconnect ${reconnectAttempts}/${MAX_TOTAL_ATTEMPTS} in 10 minutes`);
            scheduleReconnect(delay);
        } else {
            console.error(`[Background] ⛔ Max reconnect attempts (${MAX_TOTAL_ATTEMPTS}) reached. Stopping reconnect loop to save resources.`);
        }
    };
...
// Chrome MV3 Service Worker Keep-Alive (Chống đi ngủ ngầm)
try {
  chrome.alarms.create("keepAliveWS", { periodInMinutes: 0.5 });
  chrome.alarms.onAlarm.addListener((alarm) => {
    if (alarm.name === "keepAliveWS") {
      if (!ws || ws.readyState !== WebSocket.OPEN) {
        console.log("[Background] 💓 KeepAlive alarm triggered reconnect check...");
        connectWS();
      }
    }
  });
