...
export class RateLimiter {
    ...
    async acquire(): Promise<RateLimitResult> {
        ...
        if (now < this.wafCooldownUntil) {
            ...
            console.log(`[RateLimiter] 🚫 WAF cooldown active. ${Math.ceil(retryAfterMs / 1000)}s remaining. Resetting limits and exiting...`);
            this.reset();
            process.exit(0);
        }
        ...
        if (this.minuteTimestamps.length >= this.config.maxRequestsPerMinute) {
            console.log(`[RateLimiter] 🚫 Per-minute rate limit exceeded (${this.minuteTimestamps.length}/${this.config.maxRequestsPerMinute}). Resetting limits and exiting...`);
            this.reset();
            process.exit(0);
        }
        ...
        if (this.hourTimestamps.length >= this.config.maxRequestsPerHour) {
            console.log(`[RateLimiter] 🚫 Per-hour rate limit exceeded (${this.hourTimestamps.length}/${this.config.maxRequestsPerHour}). Resetting limits and exiting...`);
            this.reset();
            process.exit(0);
        }
